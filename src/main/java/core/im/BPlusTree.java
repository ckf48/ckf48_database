...
public class BPlusTree {
    ...
    public static long create(DataManager dm) throws Exception {
        ...
        long rootUid = dm.insert(TransactionManagerImp.SUPER_XID, rawRoot);
        return dm.insert(TransactionManagerImp.SUPER_XID, Parser.long2Byte(rootUid));
    }
    ...
    private long rootUid() {
        ...
        try {
            ...
            return Parser.parserLong(Arrays.copyOfRange(sa.data, sa.start, sa.start + 8));
        } finally {
            bootLock.unlock();
        }
    }
    ...
    private void updateRootUid(long left, long right, long rightKey) throws Exception {
        ...
        try {
            ...
            long newRootUid = dm.insert(TransactionManagerImp.SUPER_XID, rootRaw);
            bootDataItem.before();
            SubArray diRaw = bootDataItem.data();
            System.arraycopy(Parser.long2Byte(newRootUid), 0, diRaw.data, diRaw.start, 8);
            bootDataItem.after(TransactionManagerImp.SUPER_XID);
        } finally {
            bootLock.unlock();
        }
    }
